...
$terms = [
    ...
    [
        ...
        'slug'       => 'quantizer',
        'category'   => 'vcvrack',
        'definition' => 'Módulo que fuerza los voltajes de entrada a ajustarse a las notas exactas de una escala musical seleccionada, asegurando que siempre se esté afinado. Esencial cuando se usan fuentes de CV continuas (como LFOs o S&H) para generar melodías.',
    ],
    [
        'term'       => 'Aliasing',
        'slug'       => 'aliasing',
...
<main id="primary" class="bg-slate-950 text-slate-50">
    ...
    <section class="relative overflow-hidden px-6 sm:px-10 py-16 sm:py-20 text-center">
        ...
        <div class="absolute top-0 right-0 -mr-40 -mt-40 w-[600px] h-[600px] bg-primary/15 rounded-full blur-[120px]"></div>
        <div class="absolute bottom-0 left-0 -ml-40 -mb-40 w-[500px] h-[500px] bg-emerald-500/10 rounded-full blur-[100px]"></div>
        ...
        <div class="relative z-10 max-w-4xl mx-auto space-y-5">
            ...
            <p class="text-lg sm:text-xl text-slate-300 leading-relaxed max-w-3xl mx-auto">
                Términos esenciales de síntesis, mezcla, audio digital, VCV Rack y Bitwig Studio. Tu referencia rápida para entender el lenguaje de la producción.
            </p>
        </div>
    </section>
    ...
    <section class="max-w-5xl mx-auto px-6 -mt-6 relative z-10">
        ...
        <div class="flex gap-3 mb-6">
            <div class="relative flex-1">
                <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path stroke-linecap="round" d="m21 21-4.35-4.35"/></svg>
                <input
                    type="text"
                    id="glosario-search"
            ...
            <button type="button" id="glosario-theme-toggle" class="glosario-theme-btn shrink-0 w-12 h-12 flex items-center justify-center rounded-xl border border-slate-700 bg-slate-900 text-slate-400 hover:text-white hover:border-slate-600 transition-colors" aria-label="Cambiar tema claro/oscuro" title="Modo día / noche">
        ...
        <div class="max-w-5xl mx-auto px-6 py-2 flex flex-wrap justify-center gap-1">
            <?php foreach ($alphabet as $letter): ?>
                <a
                    ...
                    <?php if (!isset($grouped[$letter])): ?>aria-disabled="true"<?php endif; ?>
                ><?php echo $letter; ?></a>
            <?php endforeach; ?>
        </div>
    </div>
    ...
    <section class="max-w-6xl mx-auto px-6 py-10" id="glosario-list">
        <?php foreach ($alphabet as $letter): ?>
            <?php if (isset($grouped[$letter])): ?>
                <div class="glosario-group mb-8" data-letter="<?php echo $letter; ?>">
                    <h2 id="letter-<?php echo $letter; ?>" class="text-xl font-extrabold text-primary mb-4 scroll-mt-16 tracking-wide"><?php echo $letter; ?></h2>
                    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                        ...
                        ?>
                            ...
                            >
                                <div class="glosario-icon w-12 h-12 rounded-xl <?php echo esc_attr($cat['bg']); ?> flex items-center justify-center transition-transform duration-200 group-hover:scale-110">
                                    <svg class="w-6 h-6 <?php echo esc_attr($cat['text']); ?>" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><?php echo $svg_inner; ?></svg>
                                </div>
                                <span class="glosario-card-name font-bold text-sm text-white leading-tight group-hover:text-primary transition-colors"><?php echo esc_html($t['term']); ?></span>
                                <span class="inline-block px-2 py-0.5 rounded-full text-[10px] font-semibold border <?php echo esc_attr($cat['bg'] . ' ' . $cat['text'] . ' ' . $cat['border']); ?>">
                                    <?php echo esc_html($cat['label']); ?>
                                </span>
                            </button>
                        <?php endforeach; ?>
        ...
        <div id="glosario-empty" class="hidden text-center py-16">
            <svg class="w-12 h-12 mx-auto text-slate-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"/></svg>
    ...
    <div id="glosario-modal" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-lg bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 sm:p-8 max-h-[85vh] overflow-y-auto">
            <button type="button" id="glosario-modal-close" class="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full text-slate-400 hover:text-white hover:bg-slate-800 transition-colors">
            ...
            <div class="space-y-4">
                <div class="flex items-start gap-4">
                    ...
                    <div class="space-y-2 min-w-0">
                        ...
                        <span id="modal-badge" class="inline-block px-3 py-1 rounded-full text-xs font-semibold border"></span>
                    </div>
                </div>
...
<style>
    ...
    .glosario-light .glosario-input::placeholder { color: #94a3b8; }
    /* Theme toggle button */
    ...
    .glosario-light .bg-blue-500\/15    { background-color: rgba(59,130,246,0.1); }
    .glosario-light .text-blue-400       { color: #2563eb; }
    .glosario-light .border-blue-500\/25 { border-color: rgba(59,130,246,0.2); }
    .glosario-light .bg-orange-500\/15    { background-color: rgba(249,115,22,0.1); }
    .glosario-light .text-orange-400       { color: #ea580c; }
    .glosario-light .border-orange-500\/25 { border-color: rgba(249,115,22,0.2); }
    .glosario-light .bg-emerald-500\/15    { background-color: rgba(16,185,129,0.1); }
    .glosario-light .text-emerald-400       { color: #059669; }
    .glosario-light .border-emerald-500\/25 { border-color: rgba(16,185,129,0.2); }
    .glosario-light .bg-purple-500\/15    { background-color: rgba(168,85,247,0.1); }
    .glosario-light .text-purple-400       { color: #7c3aed; }
    .glosario-light .border-purple-500\/25 { border-color: rgba(168,85,247,0.2); }
    .glosario-light .bg-slate-500\/15    { background-color: rgba(100,116,139,0.1); }
    .glosario-light .text-slate-400       { color: #475569; }
    .glosario-light .border-slate-500\/25 { border-color: rgba(100,116,139,0.2); }
    /* Empty state */
    .glosario-light #glosario-empty p:first-child { color: #64748b; }
    .glosario-light #glosario-empty p:last-child  { color: #94a3b8; }
    .glosario-light #glosario-empty svg            { color: #cbd5e1; }
    /* Modal */
    ...
    .glosario-light .glosario-icon-moon { display: none; }
    .glosario-light .glosario-icon-sun  { display: block; }
</style>
